...
def _download(name):
    ...
    with st.spinner(f"Downloading {name} from Google Drive… (first load only, please wait)"):
...
def _get(name):
    ...
    if path:
        return path          # found locally (works on your machine as-is)
    if name in DRIVE_IDS:
        return _download(name)   # not local → pull from Drive (Streamlit Cloud)
