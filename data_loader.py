...
def load_raw():
    paths = {
        "orders":          _find("orders.csv"),
        "products":        _find("products.csv"),
        "order_products":  _find("order_products__prior.csv"),
        "aisles":          _find("aisles.csv"),
        "departments":     _find("departments.csv"),
        "train":           _find("order_products__train.csv"),
    }
    ...
    order_products = pd.read_csv(paths["order_products"])
    aisles         = pd.read_csv(paths["aisles"])
    departments    = pd.read_csv(paths["departments"])
    train          = pd.read_csv(paths["train"])
    return dict(orders=orders, products=products,
                order_products=order_products, aisles=aisles,
                departments=departments, train=train), []
...
def load_combined():
    ...
    df = (raw["order_products"]
          ...
          .merge(raw["products"], on="product_id",  how="left"))
...
def load_apriori_rules():
    ...
    try:
        ...
        sample_orders = filtered["order_id"].drop_duplicates().head(5000)
...
def load_model():
    ...
    try:
        ...
        train_users = np.random.choice(uniq_train_users,
                                        size=int(0.7*len(uniq_train_users)),
                                        replace=False)
        test_users  = np.setdiff1d(uniq_train_users, train_users)
        ...
        def make_dataset(user_list):
            ...
            last = (orders.query("user_id in @user_list and eval_set == 'train'")
                    .merge(train_df, on="order_id")[["user_id","product_id"]]
                    .assign(target=1))
            final = (base.merge(last, on=["user_id","product_id"], how="left")
                     .fillna({"target":0})
