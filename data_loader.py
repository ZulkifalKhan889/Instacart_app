...
# ── Optimized CSV readers (minimise RAM usage) ────────────────────────────────
# Streamlit Cloud free tier = 1 GB RAM
# Strategy: load only needed columns, cast to smallest possible dtypes
...
def load_combined():
    ...
    # Only merge the prior orders (not train) for EDA
    df = (
        raw["order_products"]
        .merge(
...
def load_model():
    ...
    try:
        ...
        # Sample only 50% of users to keep memory under 1 GB on Streamlit Cloud
        sample_size = min(int(0.5 * len(uniq_train_users)), 20000)
        sampled_users = np.random.choice(uniq_train_users, size=sample_size, replace=False)
        train_users = sampled_users[:int(0.7 * len(sampled_users))]
        test_users  = sampled_users[int(0.7 * len(sampled_users)):]
